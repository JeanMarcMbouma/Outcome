...
namespace BbQ.Cqrs.SourceGenerators
{
    ...
    public class HandlerRegistrationGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            // Generate registration extension methods
            context.RegisterSourceOutput(allHandlers.Combine(allBehaviors),
                static (spc, data) => GenerateRegistrationExtensions(spc, data.Left, data.Right));
        }
            ...
            IEnumerable<BehaviorInfo?> behaviors)
        {
            ...
            sb.AppendLine("    /// Auto-generated extension methods for registering CQRS handlers and behaviors.");
            sb.AppendLine("    /// </summary>");
            ...
            {
                sb.AppendLine("        /// <summary>");
                sb.AppendLine("        /// Registers all auto-detected command and query handlers.");
                sb.AppendLine("        /// </summary>");
            ...
            if (validBehaviors.Count > 0)
            {
                sb.AppendLine();
                sb.AppendLine("        /// <summary>");
                sb.AppendLine("        /// Registers all behaviors marked with [Behavior] attribute in order.");
                sb.AppendLine("        /// </summary>");
            ...
            if (validHandlers.Count > 0 || validBehaviors.Count > 0)
            {
                sb.AppendLine();
                sb.AppendLine("        /// <summary>");
                sb.AppendLine("        /// Registers all auto-detected handlers and behaviors.");
                sb.AppendLine("        /// </summary>");
                sb.AppendLine("        public static IServiceCollection AddGeneratedCqrs(this IServiceCollection services)");
                sb.AppendLine("        {");
                ...
                if (validBehaviors.Count > 0)
                {
                    sb.AppendLine("            services.AddGeneratedBehaviors();");
                }
            ...
            context.AddSource("GeneratedRegistrationExtensions.g.cs", sb.ToString());
        }
